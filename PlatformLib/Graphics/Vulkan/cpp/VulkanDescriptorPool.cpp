...
bool VulkanDescriptorPool::Create(revDevice* device, uint32 cbufferResourceMaxNum, uint32 textureResourceMaxNum)
{
    ...
    descriptorPoolSize[static_cast<uint32>(DESCRIPTOR_HEAP_TYPE::RESOURCE)].descriptorCount = cbufferResourceMaxNum;
    descriptorPoolSize[static_cast<uint32>(DESCRIPTOR_HEAP_TYPE::RESOURCE)].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;

    descriptorPoolSize[static_cast<uint32>(DESCRIPTOR_HEAP_TYPE::SAMPLER)].descriptorCount = textureResourceMaxNum;
    descriptorPoolSize[static_cast<uint32>(DESCRIPTOR_HEAP_TYPE::SAMPLER)].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
